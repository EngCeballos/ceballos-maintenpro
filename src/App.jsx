...
function LoginPage({onLogin}) {
  ...
  const getAllUsers = () => {
    try { const s=JSON.parse(localStorage.getItem("maintenpro_users")||"[]"); return [...BASE_USERS,...s]; }
  ...
  const handleLogin = async () => {
    ...
    if(encontrado){
      if(encontrado.senha===senha){ setLoading(false); onLogin(encontrado); return; }
      else { setErro("Senha incorreta. Verifique e tente novamente."); setLoading(false); return; }
    }
      ...
      <div style={{padding:"16px 14px",animation:"fadeUp .2s ease"}}>
        ...
        {user.perfil==="operador"&&(
          <>
            {tab==="novo"&&(
              <>
                <SeletorTipo tipo={tab==="inspecao_form"?"inspecao":tab==="os_form"?"os":"manutencao"} onChange={t=>{ if(t==="os") setTab("os_form"); else if(t==="inspecao") setTab("inspecao_form"); else setTab("novo"); }}/>
                <OperadorForm user={user} records={records} setRecords={setRecords}/>
              </>
            )}
